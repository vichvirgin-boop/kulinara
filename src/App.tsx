...
function loadDB() {
  try {
    ...
    // Important: a parse error means the stored data is corrupted, not absent.
    // We do NOT silently wipe it — we keep the raw string around in case the
    // person wants to recover it, and we surface this loudly in the console.
...
// Merge imported data into current db, de-duplicating by id.
function mergeBackup(currentDB, incoming) {
  ...
  const newRecipes      = incoming.recipes.filter((r) => !existingRecipeIds.has(r.id))
